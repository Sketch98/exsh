...
const Triev = struct { bit_string: u32 = 0, kids: []*Triev };
...
pub fn main() anyerror!void {
    ...
    inline while (i < kids.len) : (i += 1) {
        kids[i] = &Triev{ .bit_string = i * i, .kids = empty_arr[0..] };
    }
    const triev = Triev{ .bit_string = 0xFFFF_FFFF, .kids = kids[0..] };
    for (triev.kids) |kid| {
        std.debug.print("{} {any}\n", .{ kid.bit_string, kid.kids });
    }
